...
def functionality():
    ...
    st.markdown('<p class="urdu-font"; style=text-align:right; >:درست لفظ اعراب کے ساتھ یہاں درج کیجیے</p>', unsafe_allow_html=True)
    incorr=st.text_input('')
    st.markdown('<p class="urdu-font"; style=text-align:right; >:مزید اِملا شامِل کیجیے (  ہر لفظ لکھنے کے بعد نیا لفظ انٹر دبا کر تحریر کیجیے)</p>', unsafe_allow_html=True)
    var=st.text_area('')
